        ...
        switch ($_GET['action']) {
            case 'readAll':
                if ($result['dataset'] = $tipo_doc->readAllNiv()) {
                    ...
                    $result['exception'] = 'There is no level registered';
                }
                break;
            case 'search':
            ...
            case 'create':
                ...
                if($tipo_doc->setNivel($_POST['nombreTipo'])){
                    if ($tipo_doc->createNiv()) {
                        $result['status'] = 1;
                        $result['message'] = 'Addes correctly';
                    } else {
            ...
            case 'readOne':
                if ($tipo_doc->setId($_POST['id_tipodocente'])) {
                    if ($result['dataset'] = $tipo_doc->readOneNivel()) {
                        $result['status'] = 1;
                    } else {
                        $result['exception'] = 'Incorrect';
                    }

...
require_once('../../models/estadoalumno.php');
            ...
            case 'create':
                ...
                if($estado->setNombreestado($_POST['nombreestado'])){
                    if ($estado->createEstado()) {
                        $result['status'] = 1;
                        $result['message'] = 'Estado grado agregado correctamente';
                    } else {
                ...
                break;
            case 'readOne':
                if ($estado->setId($_POST['id_estadoalumno'])) {
                    if ($result['dataset'] = $estado->readOneEstado()) {
                        $result['status'] = 1;
                    } else {
                        $result['exception'] = 'Estado grado inexistente';
                    }
                } else {
                    ...
                    if ($data = $estado->readOneEstado()) {
                        if ($estado->setNombreestado($_POST['nombreestado'])) {
                                    if ($estado->updateEstado()) {
                                        $result['status'] = 1;
                                        $result['message'] = 'Estado grado modificado correctamente';
                                    } else {
                                        $result['exception'] = Database::getException();
                                    }
                                }
            ...
            case 'delete':
                if ($estado->setId($_POST['id_estadoalumno']) ) {
                    if ($estado->deleteEstado()) {
                        $result['status'] = 1;
                        $result['message'] = 'Estado grado eliminado correctamente';
                    } else {
                        $result['exception'] = Database::getException();
                    }    
                } else {
